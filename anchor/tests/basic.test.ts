...
describe('Watchlist Program', () => {
  ...
  it('initializes a watchlist', async () => {
    await program.methods.initialize().accounts({ authority: authority.publicKey }).rpc()
  ...
  it('adds a protocol', async () => {
    await program.methods.addProtocol('raydium').accounts({ authority: authority.publicKey }).rpc()
  ...
  it('rejects duplicates', async () => {
    await expect(
      program.methods.addProtocol('raydium').accounts({ authority: authority.publicKey }).rpc(),
    ).rejects.toThrow(/AlreadyExists/)
  })

  it('adds a second protocol', async () => {
    await program.methods.addProtocol('orca').accounts({ authority: authority.publicKey }).rpc()
  ...
  it('removes a protocol', async () => {
    await program.methods.removeProtocol('raydium').accounts({ authority: authority.publicKey }).rpc()
  ...
  it('rejects removing non-existent protocol', async () => {
    await expect(
      program.methods.removeProtocol('does-not-exist').accounts({ authority: authority.publicKey }).rpc(),
