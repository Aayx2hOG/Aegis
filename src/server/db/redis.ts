...
export const redis = redisUrl ? (global.__redis ?? new Redis(redisUrl, redisOptions)) : null
...
export async function publishAlertEvent(type: 'EVENT_CREATED' | 'SUMMARY_COMPLETED', event: StreamAlertEvent) {
  if (!redis) return
  try {
