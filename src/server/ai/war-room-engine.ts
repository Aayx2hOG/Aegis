...
function runMonteCarlo(positions: PortfolioPosition[], scenario: ScenarioConfig, trials = 500, steps = 10): MonteCarloMetrics {
    const initialValue = positions.reduce((acc, p) => acc + p.usdValue, 0);
    if (initialValue === 0) {
        return { averageEndingValue: 0, var95Usd: 0, drawdown95Pct: 0, liquidationProbability: 0, averageSlippageLossUsd: 0 };
    }
    ...
    for (let t = 0; t < trials; t++) {
        ...
        for (let s = 0; s < steps; s++) {
            for (const posInfo of posValues) {
                ...
                const stepSlippagePct = liquidityPenaltyFraction * ((100 - p.liquidityScore) / 100) * LIQUIDITY_SLIPPAGE_MULT / steps;
...
function riskFromPositions(positions: PortfolioPosition[], scenario: ScenarioConfig): RiskBreakdown {
    ...
    const marketRisk = clamp(
        avgVol + Math.abs(scenario.marketShockPct) * MARKET_SHOCK_MULT,
        0,
        100
    );
    ...
    const concentrationRisk = clamp(
        hhi * 100 + scenario.protocolExploitSeverity * CONCENTRATION_SCENARIO_MULT,
        0,
        100
    );
    ...
    const smartContractRisk = clamp(
        (positions.reduce((acc, p) => acc + p.usdValue * (p.kind === 'lp' ? 0.8 : p.kind === 'lending' ? 0.7 : 0.45), 0) / total) *
        100 *
        SMART_CONTRACT_BASE_MULT +
