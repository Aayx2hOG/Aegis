...
function runMonteCarloForChain(
  ...
  const bridgeLossPct = isBridgeAffected ? Math.min((scenario.bridgeOutageDurationMinutes ?? 0) / 10, 50) / 100 : 0
 ...
 */
function calculateBridgeRisk(bridgeDurationMinutes: number = 0, positions: ChainPortfolioPosition[]): number {
...
function calculateChainRisk(
  ...
  const liquidationRisk = clamp(sim.liquidationProbability + Math.abs(scenario.marketShockPct) * 1.1, 0, 100)
...
function generateRebalancingRecommendations(
  ...
  const highRiskPositions = portfolio.positions.filter((p) => p.chain === highRiskChain.chain)
 ...
 */
function detectArbitrageOpportunities(portfolio: MultiChainPortfolio): ArbitrageOpportunity[] {
...
function generateAiBriefing(
  ...
  if (scenario.marketShockPct > 0) {
    activeThreats.push(
      `Market Shock of -${scenario.marketShockPct}% simulated. Volatile assets are exhibiting high-beta drawdown correlations.`,
    )
  }
  if (scenario.liquidityDropPct > 0) {
    activeThreats.push(
      `Liquidity contraction of ${scenario.liquidityDropPct}% detected. High slippage penalizing quick exits.`,
    )
  }
  if (scenario.protocolExploitSeverity > 0) {
    activeThreats.push(
      `Lending/Yield exploit risk simulation level: ${scenario.protocolExploitSeverity}/10. High-risk smart contracts flagged.`,
    )
  }
  if (scenario.bridgeOutageDurationMinutes && scenario.bridgeOutageDurationMinutes > 0) {
    activeThreats.push(
      `Cross-chain bridge outage simulated for ${scenario.bridgeOutageDurationMinutes} minutes. Liquidity locked on: ${(scenario.chainsAffected ?? []).join(', ')}.`,
    )
  }
  if (scenario.oracleDelayMinutes > 0) {
    activeThreats.push(
      `Oracle update latency simulation of ${scenario.oracleDelayMinutes} minutes. Liquidation delay penalties applied.`,
    )
  }
...
export function runComparativeSimulation(
  ...
  const riskByChain = Object.values(ChainType).reduce(
    (acc, chainType) => {
      acc[chainType] = 0
      return acc
    },
