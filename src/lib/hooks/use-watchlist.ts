...
function getWatchlistPda(walletPubkey: PublicKey, programId: PublicKey) {
  return PublicKey.findProgramAddressSync([Buffer.from('watchlist'), walletPubkey.toBuffer()], programId)[0]
}
...
export function useWatchlist() {
    ...
    queryFn: async () => {
      ...
      try {
        const results = await Promise.all(
          geckoIds.map((id) =>
            fetchJson<CoinGeckoResponse>(`/api/coingecko?id=${encodeURIComponent(id)}`).catch(() => null),
          ),
        )
  ...
  const initialize = useMutation({
    mutationFn: async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      return (program.methods as any).initialize().accounts({ authority: wallet.publicKey }).rpc()
    },
    onSuccess: (signature) => {
  ...
  return {
    ...
    toggle: (slug: string) => {
      const normalizedSlug = normalizeProtocolSlug(slug)
      return (watchlist ?? []).includes(normalizedSlug) ? remove.mutate(normalizedSlug) : add.mutate(normalizedSlug)
    },
    needsInit: false,
