...
export function AlertRulesList({
  ...
  const [rulePendingDelete, setRulePendingDelete] = useState<AlertRuleItem | null>(null)
  const deleteInProgress = rulePendingDelete?.id === deletingRuleId

  async function confirmDelete() {
    if (!rulePendingDelete) return
    await onDeleteRule(rulePendingDelete)
  ...
  return (
    <>
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open && !deleteInProgress) setRulePendingDelete(null)
        }}
      >
        <DialogContent className="border-rose-400/20 bg-zinc-950 text-zinc-100 sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="font-orbitron text-base tracking-wide">Delete alert rule?</DialogTitle>
            <DialogDescription className="font-mono text-xs leading-relaxed text-zinc-400">
              Monitoring for <span className="font-semibold text-zinc-100">{rulePendingDelete?.protocolSlug}</span> will
              stop immediately. This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="mt-2">
            <DialogClose asChild>
              <Button type="button" variant="outline" disabled={deleteInProgress}>
                Cancel
              </Button>
            </DialogClose>
            <Button
            ...
            >
              {deleteInProgress ? 'Deleting…' : 'Delete alert'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
