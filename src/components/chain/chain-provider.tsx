...
export function MultiChainProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<ChainProviderContext>(
    ...
    [activeChain, sortedChains, activeConnections, setActiveChain, setChains, setActiveConnections, chains],
  )
...
export function useChain(chainType: ChainType, environment?: ChainEnvironment) {
  ...
  return allChains.find((c) => c.type === chainType && (!environment || c.environment === environment))
}
