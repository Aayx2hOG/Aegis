    ...
    const runExecutionLogs = async () => {
      ...
      if (fetchedTasks.length > 0) {
        ...
        fetchedTasks.forEach((task) => {
          ...
          const taskColor = task.status === 'FAILED' ? '[ERROR]' : task.status === 'SENT' || task.status === 'COMPLETED' ? '[OK]' : '[WARNING]';
          taskLogs.push(`${taskColor} TASK [${task.type}] - Target: ${task.target} - Status: ${task.status} (${timeStr})`);
        ...
        {showReceipt && status === 'success' && (
          <div className="mb-4 p-4 border border-cyan-500/20 bg-zinc-950/90 rounded-md font-mono text-[11px] text-zinc-300 space-y-2 select-text animate-in slide-in-from-top-4 duration-300">
            <div className="flex items-center justify-between border-b border-zinc-900 pb-1.5 mb-2 select-none">
              <span className="font-bold text-cyan-400 uppercase tracking-widest text-[9px]">Aegis System Audit Log</span>
            ...
            <div className="grid grid-cols-3 gap-y-1.5 gap-x-2 text-left">
              ...
              <div className="col-span-2 text-zinc-200 uppercase">{action?.action} {action?.amount} {action?.assetSymbol}</div>

              <div className="text-zinc-500 select-none">Target Network:</div>
              <div className="col-span-2 text-zinc-200 uppercase">{action?.fromChain ?? 'solana'} {action?.toChain ? `-> ${action.toChain}` : ''}</div>
        ...
        <div className="space-y-4">
          <div className="space-y-1 text-left select-none">
            ...
            <p className="text-[9px] font-mono text-zinc-500 text-left select-none mt-1">
              * Displaying actual background task logs, alert evaluator records, and notification history from the database.
            </p>
          </div>
