...
function SearchableDropdown({
  ...
  const filteredOptions = useMemo(() => {
    ...
    return options.filter((opt) => opt.label.toLowerCase().includes(query) || opt.slug.toLowerCase().includes(query))
  }, [options, search])
  ...
  return (
    <div className="space-y-2 relative text-left" ref={dropdownRef}>
      <label className="text-xs font-orbitron font-bold uppercase tracking-wider text-cyan-400/80 block">{label}</label>
        ...
        <span className="truncate">
          {selectedOption ? (
            <span className="font-semibold text-zinc-100">
              ...
              <span className="text-cyan-400 font-normal">(TVL: {formatTvl(selectedOption.tvl)})</span>
            </span>
          ) : (
        ...
        <div className="absolute left-0 right-0 z-50 mt-1.5 rounded-xs border border-cyan-500/20 bg-[#070b13] p-2.5 shadow-2xl backdrop-blur-md max-h-[450px] flex flex-col">
          ...
          <div className="overflow-y-auto flex-1 divide-y divide-zinc-900 pr-1 max-h-[350px] scrollbar-thin">
            ...
            {filteredOptions.length === 0 && (
              <div className="p-4 text-center text-xs text-zinc-550 font-mono">
                &gt; Vector mismatch. Try another query.
              </div>
            )}
          </div>
        </div>
...
function CompareContent() {
  ...
  const supportedProtocols = useMemo(() => {
    ...
    return Array.from(deduped.values())
      ...
      .map((p) => {
        const tvlVal =
          (p as unknown as { tvl?: number; tvlUsd?: number }).tvl ??
          (p as unknown as { tvl?: number; tvlUsd?: number }).tvlUsd ??
          0
  ...
  const metrics = useMemo(() => {
    ...
    const tvlCallA = brief.toolCalls.find(
      (tc) => tc.tool === 'get_protocol_tvl' && normalizeProtocolSlug(String(tc.input?.slug ?? '')) === normA,
    )
    const tvlCallB = brief.toolCalls.find(
      (tc) => tc.tool === 'get_protocol_tvl' && normalizeProtocolSlug(String(tc.input?.slug ?? '')) === normB,
    )
    const snapCallA = brief.toolCalls.find(
      (tc) => tc.tool === 'get_protocol_snapshot' && normalizeProtocolSlug(String(tc.input?.slug ?? '')) === normA,
    )
    const snapCallB = brief.toolCalls.find(
      (tc) => tc.tool === 'get_protocol_snapshot' && normalizeProtocolSlug(String(tc.input?.slug ?? '')) === normB,
    )
        ...
        <div className="animate-in slide-in-from-bottom-4 fade-in overflow-hidden rounded-xl bg-zinc-950 border border-cyan-500/15 backdrop-blur-md duration-500 relative min-h-[16rem] flex flex-col justify-between shadow-2xl corner-decor">
          <div className="border-b border-cyan-500/15 bg-zinc-900/50 px-4 py-2 flex items-center justify-between text-cyan-400/80 font-mono text-xs">
            <span className="flex items-center gap-2">
              <Terminal className="w-3.5 h-3.5" /> DECOMPOSING DUAL VECTORS
            </span>
            <span className="text-zinc-550 flex items-center gap-1.5">
              <RefreshCw className="w-3 h-3 animate-spin" /> COMPARISON: RUNNING
            </span>
          </div>
          ...
          <div className="border-t border-cyan-500/10 p-3 bg-cyan-950/5 text-center">
            <p className="text-[10px] uppercase font-orbitron font-bold tracking-widest text-cyan-500/60">
              AUDITING CONTRACT STATES IN PARALLEL
            </p>
          </div>
        </div>
      )}
      ...
      {brief && metrics && !loading && (
        <div className="animate-in fade-in duration-700 space-y-8 text-left">
          ...
          <section className="grid gap-6 md:grid-cols-2">
            ...
            >
              <span className="text-[9px] font-orbitron font-bold uppercase tracking-widest text-cyan-400 bg-cyan-950/30 border border-cyan-500/20 px-2.5 py-0.5 rounded-xs">
                VECTOR A
              </span>
              <h2 className="text-2xl font-orbitron font-black text-white mt-4 capitalize tracking-wide">
                {metrics.nameA}
              </h2>
              ...
              <div className="mt-6">
                <Button
                  asChild
                  className="border border-cyan-500/15 bg-cyan-500/5 text-cyan-400 hover:text-white hover:bg-cyan-500/20 w-full h-10 rounded-xs font-orbitron font-bold uppercase tracking-wider cursor-pointer"
                >
          ...
          <SpotlightCard
            ...
            className="rounded-xl border border-cyan-500/10 bg-zinc-950/50 p-6"
          >
                ...
                return (
                  <div className="space-y-2 text-left">
                    ...
                    <div className="h-2.5 w-full bg-zinc-950 border border-zinc-900 rounded-xs overflow-hidden flex">
                      <div
                        className="h-full bg-gradient-to-r from-cyan-600 to-cyan-400 shadow-[0_0_10px_rgba(6,182,212,0.3)] transition-all duration-500"
                        style={{ width: `${pctA}%` }}
                      />
                      <div
                        className="h-full bg-gradient-to-r from-zinc-700 to-zinc-800 transition-all duration-500"
                        style={{ width: `${pctB}%` }}
                      />
                    </div>
                  </div>
            ...
            <Table className="font-mono text-xs text-left">
              <TableHeader>
                <TableRow className="hover:bg-transparent border-zinc-850">
                  <TableHead className="text-zinc-500 uppercase text-[9px] font-bold tracking-wider">
                    Telemetry Dimension
                  </TableHead>
                  <TableHead className="text-cyan-400 uppercase text-[9px] font-bold tracking-wider">
                    {metrics.nameA}
                  </TableHead>
                  <TableHead className="text-cyan-400 uppercase text-[9px] font-bold tracking-wider">
                    {metrics.nameB}
              ...
              <TableBody>
                <TableRow className="border-zinc-900 hover:bg-zinc-900/40">
                  ...
                  <TableCell
                    className={`font-medium ${Number(metrics.tvlA) > Number(metrics.tvlB) ? 'text-white border-l-2 border-l-cyan-500 pl-3 font-semibold' : 'text-zinc-400'}`}
                  >
                    {usd(metrics.tvlA)}
                  </TableCell>
                  <TableCell
                    className={`font-medium ${Number(metrics.tvlB) > Number(metrics.tvlA) ? 'text-white border-l-2 border-l-cyan-500 pl-3 font-semibold' : 'text-zinc-400'}`}
                  >
                    {usd(metrics.tvlB)}
                  </TableCell>
                </TableRow>
                <TableRow className="border-zinc-900 hover:bg-zinc-900/40">
                  ...
                  <TableCell
                    className={`font-medium ${Number(metrics.change1dA) > Number(metrics.change1dB) ? 'text-white border-l-2 border-l-cyan-500 pl-3 font-semibold' : 'text-zinc-400'}`}
                  >
                    {pct(metrics.change1dA)}
                  </TableCell>
                  <TableCell
                    className={`font-medium ${Number(metrics.change1dB) > Number(metrics.change1dA) ? 'text-white border-l-2 border-l-cyan-500 pl-3 font-semibold' : 'text-zinc-400'}`}
                  >
                    {pct(metrics.change1dB)}
                  </TableCell>
                </TableRow>
                <TableRow className="border-zinc-900 hover:bg-zinc-900/40">
                  <TableCell className="font-semibold text-zinc-400">7D TVL Momentum</TableCell>
                  <TableCell
                    className={`font-medium ${Number(metrics.change7dA) > Number(metrics.change7dB) ? 'text-white border-l-2 border-l-cyan-500 pl-3 font-semibold' : 'text-zinc-400'}`}
                  >
                    {pct(metrics.change7dA)}
                  </TableCell>
                  <TableCell
                    className={`font-medium ${Number(metrics.change7dB) > Number(metrics.change7dA) ? 'text-white border-l-2 border-l-cyan-500 pl-3 font-semibold' : 'text-zinc-400'}`}
                  >
                    {pct(metrics.change7dB)}
                  </TableCell>
                </TableRow>
                <TableRow className="border-zinc-900 hover:bg-zinc-900/40">
          ...
          <details className="group border border-zinc-850 p-4 bg-zinc-950/30 rounded-xs">
            <summary className="flex list-none cursor-pointer items-center gap-2 text-zinc-500 transition-colors hover:text-zinc-300 font-mono text-xs">
              <div className="w-5 h-5 flex items-center justify-center rounded-sm bg-zinc-900 border border-zinc-800 group-open:rotate-180 transition-transform">
                <svg className="w-3 h-3 fill-cyan-400" viewBox="0 0 20 20">
                  <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" />
                </svg>
              </div>
              <span className="text-[10px] font-orbitron font-bold uppercase tracking-widest">
                &gt; TELEMETRY QUERY PIPELINE ({brief.toolCalls.length} TRACES)
              </span>
            </summary>
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              {brief.toolCalls.map((tc, i) => (
                <div
                  key={i}
                  className="p-4 rounded-xs bg-zinc-950/90 flex flex-col gap-2 border border-zinc-850 text-xs"
                >
                  ...
                  <div className="text-[10px] text-zinc-500 truncate font-mono italic">
                    input: {JSON.stringify(tc.input)}
                  </div>
                  {tc.error && (
                    <div className="text-[10px] text-rose-500 mt-1 font-mono uppercase font-bold">
                      Trace Error: {tc.error}
                    </div>
                  )}
                </div>
              ))}
          ...
          <div className="text-center pb-20">
            <p className="text-zinc-650 font-mono text-xs">
              &gt; HEAD-TO-HEAD SYNTHESIS COMPLETED. TELEMETRY CACHE IN EQUILIBRIUM.
            </p>
          </div>
        </div>
      )}
